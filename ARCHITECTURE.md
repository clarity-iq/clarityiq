...
- **Data retention (fixed July 25, 2026):** this agent's `platform_settings.privacy.retention_days` was found set to `-1` (unlimited retention of full audio + transcripts, `record_voice: true`, nothing scheduled for deletion) — discovered during a legal-compliance audit, contradicting the "no audio or transcript stored" claims already live on the site and in the Privacy Policy. Fixed via the ElevenLabs API: `retention_days` set to `0`, with `apply_to_existing_conversations: true` to also purge everything accumulated up to that point. Verified via a follow-up read. This setting lives on ElevenLabs' side (account/dashboard config, not code) — it doesn't get restored by redeploying the Worker or the frontend, so don't assume it's covered by anything in this repo. If the agent is ever recreated or cloned, this must be re-set explicitly; it is not the platform default.
...
- **`/log-consent` (added July 25, 2026):** writes a `consent:<session_code>` record to `TELEMETRY_KV` when the patient checks the box and continues past the "Before You Begin" gate — see §5 for why this is a separate record from `session:<session_code>` telemetry, not merged into it.
...
- **This claim covers ClarityIQ's own systems — the ElevenLabs Agent had its own separate setting, and it was wrong.** The bullet above was true for the Worker/proxy the whole time, but the live audio and transcript actually flow through ElevenLabs' Agents Platform first, which has its own independent retention configuration, unrelated to anything in this repo. A July 25, 2026 audit found that setting at `retention_days: -1` (unlimited, nothing scheduled for deletion) — see §2. Fixed the same day, retroactively. The lesson: "no audio/transcript retained" is a claim about the whole pipeline, not just the code in this repo — any future vendor added to the pipeline needs its own retention setting checked and documented here, not assumed from ClarityIQ's own side being clean.
...
- **Consent audit trail (added July 25, 2026), deliberately separate from telemetry.** The "Before You Begin" gate (§1) now requires an affirmative checkbox, not just a continue click, and records a `consent:<session_code>` entry (timestamp + the exact disclosure text shown, captured live from the DOM rather than a hardcoded copy) via the Worker's `/log-consent` route. This lives in `TELEMETRY_KV` alongside `session:` and `anomaly:` records but is its own key prefix and record shape — it's a compliance record, not part of the anonymous learning-data schema above, so don't merge it into `session:` or start treating it as fair game for the same aggregate-only handling.
