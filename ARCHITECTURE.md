...
- **Email:** `support@clarityiq.ai` runs on Zoho Mail's free plan (real inbox, not forwarding), set up directly via Wix's DNS panel (MX records `mx.zoho.com`/`mx2.zoho.com`/`mx3.zoho.com` at priorities 10/20/50, plus SPF/DKIM TXT records) — interim solution, not through Cloudflare Email Routing, since Cloudflare isn't the DNS authority yet (see above). **Future to-do, not urgent:** once the domain transfers to Cloudflare in September 2026, these same Zoho MX/SPF/DKIM records will need to be re-added in Cloudflare's DNS zone, or email will break the moment the nameserver cutover happens.
