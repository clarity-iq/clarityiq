...
- **Email:** `support@clarityiq.ai` runs on Google Workspace Business Starter, set up directly via Wix's DNS panel (MX record `smtp.google.com` priority 1, plus SPF/DKIM TXT records) — not through Cloudflare Email Routing, since Cloudflare isn't the DNS authority yet (see above). **Future to-do, not urgent:** once the domain transfers to Cloudflare in September 2026, these same Workspace MX/SPF/DKIM records will need to be re-added in Cloudflare's DNS zone, or email will break the moment the nameserver cutover happens.
